...
export const lessonIds = ["01"] as const;
...
export function parseLessonId(argv: readonly string[]) {
  if (argv.length !== 1 || argv[0] !== "01") {
    throw new Error("講義番号は 01 を1つ指定してください（例: pnpm lesson:verify 01）");
  }
  return "01" as const;
}

export function getLesson(id: string) {
  if (id !== "01") throw new Error(`未対応の講義番号です: ${id}`);
  return lessons[id];
}
...
const allowedSourcePath = "apps/app/src/routes/index.tsx";
...
function addChangeIssue(issues: string[], path: string) {
  if (isProtectedPath(path)) {
    issues.push(`${path}: 保護されたファイル`);
  } else if (path.startsWith("apps/app/src/") && path !== allowedSourcePath) {
    issues.push(`${path}: 許可されていないsourceの変更`);
  } else if (path !== allowedSourcePath) {
...
export function verifyLessonSafety(
  ...
  for (const change of changes) {
    ...
    if (!path) continue;
    addChangeIssue(issues, path);
    if (
      path === allowedSourcePath &&
